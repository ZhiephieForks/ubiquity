...
class DAOPreparedQueryAll extends DAOPreparedQuery {
	...
	public function execute($params = [], $useCache = false) {
		...
		foreach ($query as $row) {
			$object = $call($this->db, $row, $className, $invertedJoinColumns, $manyToOneQueries, $this->oneToManyFields, $this->manyToManyFields, $oneToManyQueries, $manyToManyParsers, $this->accessors, $this->transformers);
