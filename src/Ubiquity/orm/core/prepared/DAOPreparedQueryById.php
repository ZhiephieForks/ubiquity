...
class DAOPreparedQueryById extends DAOPreparedQuery {
	...
	public function execute($params = [], $useCache = false) {
		...
		if ($query && \sizeof($query) > 0) {
			$call = $this->loadObjectFromRowCallback;
			$oneToManyQueries = $manyToOneQueries = $manyToManyParsers = [];
			$className = $this->className;
			$object = $call($this->db, \current($query), $className, $this->invertedJoinColumns, $manyToOneQueries, $this->oneToManyFields, $this->manyToManyFields, $oneToManyQueries, $manyToManyParsers, $this->accessors, $this->transformers);
			if ($this->hasIncluded) {
